...
class PaperService:
    ...
    async def search_papers(self, search_query: str, max_results: int = 10) -> List[str]:
        """
        Search for papers using PubMed API with pagination.

        Args:
            search_query: Query string to search for papers
            ...
            async with self.sem:
                ...
                async with aiohttp.ClientSession() as session:
                    while len(collected_ids) < max_results:
                        page_size = min(100, max_results - len(collected_ids))  # PubMed per-page cap
                        params = {
                            "db": "pubmed",
                            "term": search_query,
                            "retmode": "json",
                        ...
                        async with session.get(search_url, params=params) as response:
                            if response.status != 200:
                                logger.error(f"Error searching papers: HTTP {response.status}")
                                break

                            data = await response.json()
                            esearch = data.get("esearchresult", {}) or {}
                            page_ids = esearch.get("idlist", []) or []
                            ...
                            # If fewer than requested were returned, we've hit the end.
