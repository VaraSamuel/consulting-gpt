...
class MedicalAssessmentService:
    ...
    async def _generate_basic_recommendations(self, problem_statement: str) -> str:
        """Generate basic medical recommendations from problem statement when no guidelines are available"""
        try:
            system_prompt = (
                "Based on this medical problem statement, provide basic medical recommendations "
                "for billing and treatment purposes. Focus on standard medical procedures and treatments "
                "typically associated with this condition. Be specific and actionable."
            )
            ...
            return "Standard medical evaluation and treatment as clinically indicated"
    ...
    async def create_medical_assessment(
        ...
        try:
            ...
            if guidelines_list:
                ...
                # Fallback to API-based method if no official guidelines
                logger.warning("No official guidelines found, using alternative API-based approach")
                fallback_guidelines = await self.get_medical_guidelines_from_pubmed(
                    medical_association, technology_name, problem_statement
                )
                if fallback_guidelines:
                    codes, recommendations = await self.extract_procedures_from_guidelines(
                        fallback_guidelines, 
                        problem_statement
                    )
                else:
                    # Last resort: Generate codes directly from problem statement
                    logger.warning("No guidelines available, generating codes from problem statement")
                    codes = await self._get_hcpcs_codes(problem_statement)
                    recommendations = await self._generate_basic_recommendations(problem_statement)
