...
class TechnologyService:
    ...
    async def create_technology(self, name: str, abstract: str = None, num_of_axes: int = 3, 
                               number_of_related_patents: int = 10, number_of_related_papers: int = 10) -> Technology:
        """
        Create a new technology record
        
        Args:
            name: Technology name
            abstract: Technology abstract/description
            num_of_axes: Number of comparison axes to generate
            number_of_related_patents: Number of patents to search for
            number_of_related_papers: Number of papers to search for
        """
        try:
            ...
            db_technology = Technology(
                ...
                number_of_related_patents=number_of_related_patents,
                number_of_related_papers=number_of_related_papers
            )
    ...
    async def search_patents(self, technology_id: int, num_patents: int = None) -> bool:
        """
        Search for patents related to a technology
        
        Args:
            technology_id: ID of the technology to search patents for
            num_patents: Maximum number of patents to retrieve (defaults to technology's setting)
        """
        try:
            # Get the technology
            technology = self.get_technology_by_id(technology_id)
            ...
            # Use the technology's configured number or the provided parameter
            patents_to_search = num_patents if num_patents is not None else (technology.number_of_related_patents or 10)
            print(f"Searching {patents_to_search} patents with keywords: {technology.search_keywords}")

            # Search patents using patent service first to ensure we have results
            search_results = await self.patent_service.search_patents(technology.id, technology.search_keywords, patents_to_search)
    ...
    async def search_related_papers(self, technology_id: int, num_papers: int = None) -> bool:
        """
        Search and store related papers for a technology with complete details
        
        Args:
            technology_id: ID of the technology to search papers for
            num_papers: Maximum number of papers to retrieve (defaults to technology's setting)
        """
        try:
            # Get the technology
            technology = self.get_technology_by_id(technology_id)
            ...
            # Use the technology's configured number or the provided parameter
            papers_to_search = num_papers if num_papers is not None else (technology.number_of_related_papers or 10)
            logger.info(f"Searching {papers_to_search} papers with keywords: {technology.search_keywords}")
    
            # Search for papers
            paper_ids = await self.paper_service.search_papers(technology.search_keywords, papers_to_search)
            if not paper_ids:
